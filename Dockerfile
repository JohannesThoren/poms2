...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cp target/release/poms-ingestion \
       ...
       target/release/umeaenergi-adapter \
       target/release/gotland-adapter \
       ...
       target/release/skurup-adapter \
       target/release/ovikenergi-adapter \
       target/release/landskrona-adapter \
       target/release/norrtalje-adapter \
       target/release/kalmarenergi-adapter \
       /build/bin/
...
FROM runtime-base AS umeaenergi-adapter
COPY --from=builder /build/bin/umeaenergi-adapter /usr/local/bin/umeaenergi-adapter
ENTRYPOINT ["/usr/local/bin/umeaenergi-adapter"]

FROM runtime-base AS gotland-adapter
COPY --from=builder /build/bin/gotland-adapter /usr/local/bin/gotland-adapter
ENTRYPOINT ["/usr/local/bin/gotland-adapter"]

FROM runtime-base AS herrljunga-adapter
COPY --from=builder /build/bin/herrljunga-adapter /usr/local/bin/herrljunga-adapter
ENTRYPOINT ["/usr/local/bin/herrljunga-adapter"]

FROM runtime-base AS sandviken-adapter
COPY --from=builder /build/bin/sandviken-adapter /usr/local/bin/sandviken-adapter
ENTRYPOINT ["/usr/local/bin/sandviken-adapter"]
...
FROM runtime-base AS skurup-adapter
COPY --from=builder /build/bin/skurup-adapter /usr/local/bin/skurup-adapter
ENTRYPOINT ["/usr/local/bin/skurup-adapter"]

FROM runtime-base AS ovikenergi-adapter
COPY --from=builder /build/bin/ovikenergi-adapter /usr/local/bin/ovikenergi-adapter
ENTRYPOINT ["/usr/local/bin/ovikenergi-adapter"]

FROM runtime-base AS landskrona-adapter
COPY --from=builder /build/bin/landskrona-adapter /usr/local/bin/landskrona-adapter
ENTRYPOINT ["/usr/local/bin/landskrona-adapter"]

FROM runtime-base AS norrtalje-adapter
COPY --from=builder /build/bin/norrtalje-adapter /usr/local/bin/norrtalje-adapter
ENTRYPOINT ["/usr/local/bin/norrtalje-adapter"]

FROM runtime-base AS kalmarenergi-adapter
COPY --from=builder /build/bin/kalmarenergi-adapter /usr/local/bin/kalmarenergi-adapter
ENTRYPOINT ["/usr/local/bin/kalmarenergi-adapter"]
