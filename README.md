...
- [x] **Landskrona Energi** - WordPress med en egen post-typ (permalänkar `/avbrott/{slug}/`) som inte är REST-API-exponerad (finns varken i `/wp-json/wp/v2/types` eller som en matchande kategori/tagg) - skrapar arkiv-HTML:en direkt istället. Delar en feed med stadsnät/fjärrvärme utan kategori-fält, och **flera poster handlar om E.ON:s avbrott** (som stör Landskronas eget stadsnät/fiber-utrustning) snarare än Landskronas egen elnätsstörning - filtreras uttryckligen bort. Varje post är en rullande uppdateringslogg (senaste uppdateringen överst) - "åtgärdat"-nyckelord var som helst i texten tolkas som löst, hellre missa en aktiv post än visa en redan löst som pågående.
- [x] **Norrtälje Energi** - server-renderad direkt, en egenbyggd accordion-lista (inte SiteVision, inte WordPress) med explicit statusklass (`status-type ... finished/planned`) och kategori-tagg (`status-tag`: Elnät/Fiber/Fjärrvärme) - ovanligt rent jämfört med de flesta andra fritextbaserade källorna. Verifierat live: fångade ett riktigt planerat elnätsavbrott i Undra och Skjulsta med fullständig beskrivningstext.
...
- [ ] Bjäre Kraft `bjarekraft.se/privat/support/driftstatus` - server-renderad direkt (två flikar: Kommande/Utförda, tydlig kategori-ikon `fa-plug` vs `fa-router` för el vs internet). Begränsning: ingen egen sektion för genuint pågående/aktiva fel hittad - en post med titeln "Pågående strömavbrott" visade sig bara vara namnet på en redan avslutad historisk händelse i "Utförda"-fliken, inte en live-status. Skulle bara ge oss Kommande/planerat, inte den viktigaste kategorin (aktiva fel) - inte byggd av den anledningen. Nätverksfångst kan behövas för att hitta en dold live-status-endpoint om en sådan finns.
