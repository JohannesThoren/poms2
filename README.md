...
- [ ] Fortfarande nere (2026-09-07, verifierat att det INTE är user-agent-blockering - testat curl/Chrome/Safari/iPhone/Googlebot-UA:er samt HTTP/1.1 vs HTTP/2, identiskt resultat överallt):
  - **Jämtkraft**: `jamtkraft.se` (huvudsajt) svarar snabbt och normalt, men `avbrottskarta.jamtkraft.se` hänger helt - TLS-handskakning klar, sen ingen respons alls inom 8-10s. En separat, nere mikrotjänst, inte hela företaget.
  - **HEMAB**: hela huvudsajten (`hemab.se`) svarar 503 just nu - ett större, bredare driftstopp hos dem, inte bara `dpwebmap.hemab.se`.
  - **BTEA**: huvudsajt (`btea.se`) fungerar fint (HTTP 200), men `dpweb.btea.se` hänger på samma sätt som Jämtkraft.
  - **Härryda Energi**: huvudsajt fungerar (redirectar normalt), men `instwebb.harrydaenergi.se` hänger likadant.
  - **Hedemora Energi**: `webmap.hedemoraenergi.se` svarar snabbt med 503 och en Envoy-felkropp ("upstream connect error") - proxyn/lastbalanseraren lever och rapporterar ärligt att bakomliggande app-server är nere.
  - **Falu Elnät**: både `outagemap2/`-sidan och KML-endpointen ger HTTP 500 med en generisk SiteVision/Java-felsida (inbäddad bas64-logga) - ett äkta serverfel på deras sida, inte en blockering. `cust=fev` är fortsatt korrekt bekräftat.
  - Slutsats: inget av detta går att lösa genom att ändra hur vi frågar (UA, HTTP-version, headers) - det är riktiga driftstopp hos leverantörerna/deras Digpro-hosting. Värt att bara testa om med jämna mellanrum, inte gräva vidare i requestet.
